...
# Add an ARG to control whether to install models and extensions directly in the container
# This can be used if no network volume is to be used, however it will massively bloat
# the container size and make it slow to load.
ARG INSTALL_MODELS_AND_EXTENSIONS=false

# Conditional block for installing models, extensions, and configurations
RUN if [ "$INSTALL_MODELS_AND_EXTENSIONS" = "true" ]; then \
    WORKDIR /workspace
